...
from .CBAMblock import cbam_block as CBAM
...
class YOLOXHead(nn.Module):
    def __init__(self, num_classes, anchors_mask,width=1.0, in_channels=[256, 512, 1024], act="silu", depthwise=False, ):
        super().__init__()
        ...
        for i in range(len(in_channels)):
            ...
            self.cls_preds.append(
                nn.Conv2d(in_channels=int(256 * width), out_channels=num_classes*len(anchors_mask[0]), kernel_size=1, stride=1, padding=0)
            )
            ...
            self.reg_preds.append(
                nn.Conv2d(in_channels=int(256 * width), out_channels=4*len(anchors_mask[0]), kernel_size=1, stride=1, padding=0)
            )
            self.obj_preds.append(
                nn.Conv2d(in_channels=int(256 * width), out_channels=1*len(anchors_mask[0]), kernel_size=1, stride=1, padding=0)
            )
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi='l', pretrained=False):
        ...
        # self.head = YOLOXHead(num_classes, anchors_mask, width=1.00)
